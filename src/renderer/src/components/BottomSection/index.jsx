...
function BottomSection({
  ...
  const processQueue = useCallback(async () => {
    ...
    try {
      ...
      let info;
      try {
        info = await getVideoInfo(item.url) 
        ...
        storedDownloads[itemIndex] = {
          ...
          filename: info?.filename || `${info?.title || 'video'}.${format === 'mp3' ? 'mp3' : 'mp4'}`,
        ...
        setVideoInfo(info); 
        localStorage.setItem('downloadList', JSON.stringify(storedDownloads));
        // || await window.api.fetchVideoInfo(item.url);
      } catch (error) {
  ...
  useEffect(() => {
    const storedDownloads = JSON.parse(localStorage.getItem('downloadList') || '[]');
    const queuedDownloads = storedDownloads.filter(item => 
